...
#define sizeof_SH1106_Buffer (SH1106_WIDTH * SH1106_HEIGHT / 8)
/* SH1106 data buffer */
uint8_t SH1106_Buffer_[sizeof_SH1106_Buffer+1]; // 1 Byte for command
uint8_t* SH1106_Buffer = &SH1106_Buffer_[1]; // assign pointer for first pixer leaving command byte
...
void SH1106_I2C_WriteMulti(uint8_t address, uint8_t reg, uint8_t* data, uint16_t count) {
	...
	i2c_write(&i2c0, pixbuf, (count+1));
	while(i2c0.is_busy);
}

extern void wait_ms(uint32_t w);
...
void SH1106_UpdateScreen(void) {
	...
	for (m = 0; m < 8; m++) {
		...
		for(i=0;i<8;i++) {	
			/* Write multi data */
			SH1106_I2C_WriteMulti(SH1106_I2C_ADDR, 0x40, &SH1106_Buffer[SH1106_WIDTH * m + i*SH1106_WIDTH/8], SH1106_WIDTH/8);
		}
#endif
...
void SH1106_ToggleInvert(void) {
	...
	/* Do memory toggle */
	for (i = 0; i < sizeof_SH1106_Buffer; i++) {
...
void SH1106_Test() {
	...
	for(row=0;row<8;row++) {
		...
		for(col=0;col<128;col++) {
			if((col%8)== 0) d = ~d;		
			SH1106_Buffer[row*128 + col] = d;
		}
		d = ~d;
	}
	#endif
	//	for(i=0;i<128;i++) {
	//				SH1106_Buffer[i] = 0x81;
	//	}
}

void SH1106_Fill(SH1106_COLOR_t color) {
	/* Set memory */
	memset(SH1106_Buffer, (color == SH1106_COLOR_BLACK) ? 0x00 : 0xFF, sizeof_SH1106_Buffer);	
}
